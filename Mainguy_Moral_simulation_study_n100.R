...
## run studies for each scenario - takes approx. 50 hours in total
...
fish1 <- read.csv("equi_fish_n100.csv", header = TRUE)
fish2 <- read.csv("linear_fish_n100.csv", header = TRUE)
fish3 <- read.csv("quad_fish_n100.csv", header = TRUE)
fish4 <- read.csv("under_fish_n100.csv", header = TRUE)
...
model_est <- - all_slopes %>%
  ...
  pivot_longer(cols = 1:28,
               names_to = "model",
               values_to = "slope") %>%
  filter(model != "pn" & model != "pn1" & model != "pn2" & model != "pn3")
model_est$model <- as.factor(model_est$model)
levels(model_est$model) <- rep(c("CMP","GP","NB1","NB2","Poisson","QP"), each = 4)
model_est$scenario <- gl(4, 6, labels = c("equidispersion",
...
levels(model_se$model) <- rep(c("CMP","GP","NB1","NB2","Poisson","QP"), each = 4)
model_se$scenario <- gl(4, 6, labels = c("equidispersion",
                                         "linear overdispersion",
                                         "quadratic overdispersion",
                                         "underdispersion"))
...
weighted_se <- data.frame(scenario = c("Equi-dispersion","Linear over-dispersion","Quadratic over-dispersion","Under-dispersion"),
                          wse = c(0.02470705,0.03689285,0.09015703,0.01900242))
