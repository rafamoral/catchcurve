load("results_n100_updated.RData")
...
se_intercept1 <- apply(se1_int, 1, function(x) mean(na.omit(x)))
se_intercept2 <- apply(se2_int, 1, function(x) mean(na.omit(x)))
se_intercept3 <- apply(se3_int, 1, function(x) mean(na.omit(x)))
se_intercept4 <- apply(se4_int, 1, function(x) mean(na.omit(x)))
se_slope1 <- apply(se1_slo, 1, function(x) mean(na.omit(x)))
se_slope2 <- apply(se2_slo, 1, function(x) mean(na.omit(x)))
se_slope3 <- apply(se3_slo, 1, function(x) mean(na.omit(x)))
se_slope4 <- apply(se4_slo, 1, function(x) mean(na.omit(x)))
...
## getting AICc weights
# example using package MuMIn
#library(MuMIn)
#model.sel(object = results1$models[[1]][c(1,5,6)])

# example doing it by hand
#my_aicc <- na.omit(aicc_summary1[-7,1]) %>% as.numeric
#delta_aicc <- my_aicc - min(my_aicc)
#exp(-.5 * delta_aicc)/sum(exp(-.5 * delta_aicc))

get_aicc_weights <- function(aicc_values) {
  get_nas <- is.na(aicc_values)
  aicc_values2 <- na.omit(aicc_values) %>% as.numeric
  delta_aicc <- aicc_values2 - min(aicc_values2)
  weights <- exp(-.5 * delta_aicc)/sum(exp(-.5 * delta_aicc))
  aicc_weights <- rep(0, length(aicc_values))
  aicc_weights[!get_nas] <- weights
  return(aicc_weights)
}

all_weights1 <- apply(aicc_summary1[-7,], 2, get_aicc_weights)
all_weights2 <- apply(aicc_summary2[-7,], 2, get_aicc_weights)
all_weights3 <- apply(aicc_summary3[-7,], 2, get_aicc_weights)
all_weights4 <- apply(aicc_summary4[-7,], 2, get_aicc_weights)

all_weights <- cbind(rowMeans(all_weights1),
                     rowMeans(all_weights2),
                     rowMeans(all_weights3),
                     rowMeans(all_weights4))
colnames(all_weights) <- c("equi","lin","quad","under")
rownames(all_weights) <- c("poisson","qp","nb1","nb2","comp","gp")

all_weights

### total counts (sum y variable for each simulated dataset)
sum_y <- cbind(sapply(results1$data, function(x) sum(x$y)),
               sapply(results2$data, function(x) sum(x$y)),
               sapply(results3$data, function(x) sum(x$y)),
               sapply(results4$data, function(x) sum(x$y)))
